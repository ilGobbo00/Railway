        ...
        case park:                                                                  // Parcheggio (caso reverse e transit analizzati)
            if(is_slowing_){
                ...
                if(last_request_ != reject && last_request_ != invalid) {           // Nel caso di errori, con != invalid posso effettuare un'altra richiesta senza compromettere il sistema
                    if (last_request_ == to_platform) curr_spd_ = 80 / 60;
                    if (last_request_ == transit) curr_spd_ = 190 / 60;
                    status_ = stationMotion;
                    communications();
                }
            }
...
void Train::delay_calc(){                                                                                               // Essendo nella classe Train questa funzione verrà fatta dai treni che possono saltare le fermate, Regional ha un caso a parte
    ...
    switch (status_) {
        ...
        case stationMotion:
            delay_ = central_railw_ -> curr_time() + ceil(abs(next_stat() -> distance() - curr_km_) / (80/60)) - arrivals_[time_arrival_next_stat_];
            break;
...
// ===== CLASSE REGIONAL =====
Regional::Regional(std::string number, bool rev, Station* curr, std::vector<int> times, Railway* rail):{}
void Regional::specific_comm(){}
...
// ===== CLASSE FAST =====
Fast::Fast(std::string number, bool rev, Station* curr, std::vector<int> times, Railway* rail):{}
void Fast::specific_comm(){}
// Il treno veloce
// }								// Funzione interna invocata dal treno stesso per annunciare il suo arrivo in una stazione
...
// ===== CLASSE SUPERFAST =====
SuperFast::SuperFast(std::string number, bool rev, Station* curr, std::vector<int> times, Railway* rail):{}
void SuperFast::specific_comm(){}
