		...
		if (curr_stat_ != nullptr) {                                                                                                // Se sono appena partito dalla stazione precedente
			...
			if (last_request_ == to_delete) {                                                                                         // Se sto arrivando alla fine della linea (capolinea o non)
				...
				if ((last_request_ == transit) && (!reverse_ && curr_km_ >= next_stat_->distance()) || (reverse_ && next_stat_->distance())) {      //  sarà la successiva di quella che ha appena superato
					curr_stat_ = next_stat_;                                                                                                        // Rendo cur_stat_ diverso da nullptr perche' così il caso
...
void Train::calc_specific_delay() {
	...
	while (!principal) {                                                                                                     // Controllo finche' non trovo la stazione in cui dovrò fermarmi per calcolare il ritardo
	 ...
	 */
	if (principal) {
		station_to_skip != 0 ? delay_due_to_station = station_to_skip * ceil(3.16) : delay_due_to_station = ceil(3.75);                                              // Se e' di transito ma la prossima stazione e' una principale devo contare il tempo per fare i 5km a 80km/h
		delay_ = (central_railw_->curr_time() + ceil((abs(next_stat_->distance() - curr_km_) - 5) / curr_spd_) +
			delay_due_to_station + normal_motion_distance / max_spd_) - arrivals_[time_arrival_next_stat_];
	}
...
std::string Train::changed_delay() {
	...
	if (old_delay_ == 0) {
		if (delay_ > 0)
			return to_return += " non e' più in orario, il suo ritardo e' aumentato di " + print_delay(false) + " min\n";
		if (delay_ < 0)
			return to_return += " non e' più in orario, il suo anticipo e' aumentato di " + print_delay(false) + " min\n";
		return "";
