...
Railway::Railway(const string line_description, const string timetables) : curr_time_{ 0 }, stations_{}, trains_{}, messages_{""}	// Crea i vettori stations_ e trains_
{
    ...
    while (std::getline(trains_desc, line2))
    {
        ...
        if (t_type == 1)
        {
            ...
            if (t_times[0] < 1440)
            {
                std::cerr << "Costruzione regional\n";
                if(t_times.size() > 0) {
                    std::cerr << "Array orari non vuoto: ";
                    for (int i = 0; i < t_times.size(); i++) std::cerr << t_times[i] << " ";
                    std::cerr << "\n\n";
                }else
                    std::cerr << "Array orari vuoto\n\n";
        ...
        else
        {
            ...
            if (t_times[0] < 1440)
            {
                std::cerr << "Costruzione fast e superfast\n";
                if(t_times.size() > 0) {
                    std::cerr << "Array orari non vuoto: ";
                    for (int i = 0; i < t_times.size(); i++) std::cerr << t_times[i] << " ";
                    std::cerr << "\n\n";
